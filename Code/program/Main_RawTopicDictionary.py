...
if __name__ == '__main__':
    ...
    os.makedirs(f'./././Data/test_data/final_presentation/df_raw_{id_clustersize_similarity}', exist_ok = True)
    for n_gram_ele in n_gram_list:
        for num_topics_ele in num_topics_list:
            ...
            for topic_id, topic_terms in topics_previous:
                nouns = [word for word, probability in topic_terms]
                probabilities = [probability for word, probability in topic_terms]
            ...
            df_raw.to_csv(f'./././Data/test_data/final_presentation/df_raw_{id_clustersize_similarity}/topic_dictionary.csv', index = False, encoding = 'utf-8')
            # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

            # 生のトピック辞書を用いたTopic Tiling
            doc_topic_distributions = []
            for subdocs in subdocuments:
                sub_doc_topic_distributions = []
                for subdoc in subdocs: # subdocはn-gram
                    sub_doc_topics = calculate_topic_distribution_for_text(subdoc, df_raw)
            ...
            # トピック辞書のクラスタリング
            topic_dictionary = pd.read_csv(f'./././Data/test_data/final_presentation/df_raw_{id_clustersize_similarity}/topic_dictionary.csv')
                ...
                for similarity_threshold in similarity_thresholds:
                    for clustersize_threshold in clustersize_thresholds:
                        ...
                        for nouns, probabilities in zip(nounses, probabilitieses):
                            ...
                            # 代表クラスタの特定（クラスタが保持するベクトルの数が最大のクラスタを代表クラスタとする）
    ...
    clusters_df.to_csv(f'./././Data/test_data/final_presentation/df_raw_{id_clustersize_similarity}/word_distribution_clusters.csv', index = False, encoding = 'utf-8')
    ...
    # Topic Tilingで分割した文書に対して、生のトピック辞書を用いてトピックを割り当て
    ...
    for file_path, transcription, transcription_timestamp, topics, start_time in zip(file_paths, transcriptions, transcriptions_timestamp, topicses, start_times):
        ...
        for topic, utterance, utterance_timestamp, start, end in zip(topics_new, utterances, utterances_timestamp, start_time_new[:-1], start_time_new[1:]):
            ...
            topic_distribution = calculate_topic_distribution_for_text(utterance, df_raw)
            topic_distribution2 = []
            total = 0
    ...
    df = pd.DataFrame(data)
    df.to_csv(f'./././Data/test_data/final_presentation/df_raw_{id_clustersize_similarity}/topic_segments_info.csv', index = False, encoding = 'utf-8')
    ...
    df.to_csv(f'./././Data/test_data/final_presentation/df_raw_{id_clustersize_similarity}/topic_segments_df.csv', index = False, encoding = 'utf-8')
    ...
    topic_segment_df.to_csv(f'./././Data/test_data/final_presentation/df_raw_{id_clustersize_similarity}/topic_segments_df_new.csv', index = False, encoding = 'utf-8')

    topic_segment_df = pd.read_csv(f'./././Data/test_data/final_presentation/df_raw_{id_clustersize_similarity}/topic_segments_df_new.csv')
    print(f"トピックセグメントの数: {len(topic_segment_df)}")
    raw_data = {'threshold': [], 'raw_represent': [], 'raw_average': []}
    ...
    for i in range(1, 21):
        count = 0
        raw_data['threshold'].append(i/20)
        for degree in rdegrees:
            if float(degree) >= i/20:
                count += 1
        raw_data['raw_represent'].append(count / len(rdegrees))
    ...
    for i in range(1, 21):
        ...
        for degree in adegrees:
            if float(degree) >= i/20:
                count += 1
        raw_data['raw_average'].append(count / len(adegrees))
        print(f"{i/20}以上の割合: {count / len(adegrees):.05f}")
    print("\n")

    raw_df = pd.DataFrame(raw_data)
    raw_df.to_csv(f'./././Data/test_data/final_presentation/df_raw_{id_clustersize_similarity}/raw_results.csv', index = False, encoding = 'utf-8')
    print("Raw")
